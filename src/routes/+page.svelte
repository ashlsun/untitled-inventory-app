<script lang="ts">
  ...
  // Track if we've initialized stores for the current user
  let initializedUserId: string | null = $state(null)

  $effect(() => {
    // Only initialize stores when authenticated and not already initialized for this user
    if (auth.state === 'authenticated' && auth.user && auth.user.id !== initializedUserId) {
      const userId = auth.user.id
      initializedUserId = userId

      createItemStore('fridge', userId).then((itemStore) => {
        fridgeItems = itemStore
      })

      createItemStore('freezer', userId).then((itemStore) => {
...
{:else}
  <div class="m-2">
    <div class="flex justify-between items-center mb-2 mx-3">
      <span class="text-xs text-stone-400">{auth.user?.email}</span>
      <button
        onclick={() => auth.signOut()}
        class="text-xs text-stone-400 hover:text-stone-600 underline underline-offset-2 decoration-1"
      >
        sign out
      </button>
    </div>
    ...
    <div class="m-2 mt-12 max-w-sm">
      ...
      >
        <p>Your data syncs across devices.</p>
